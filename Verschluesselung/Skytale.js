...
let test1 = 'Spartanischer GeheimText: ein Rezept wie von Oma! 2 Löffel Salz, eine Tasse Tee.';
let test2 = 'Ein Vogel fliegt auf den Schornstein, und blickt in den Schlot hinein.';
let test3 = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
let test4 = 'Spartaner leben die Enthaltsamkeit. Ihre Feinde in der Regel nicht.';
let test5 = '22Zeichen0123456789012';
let test6 = 'abc, die Katze lief im Schnee.';
...
function unTooSkytale(n, input) {
    const uncryptArray = [];
    let cycle = Math.ceil(input.length / n);
    let give = Math.round((input.length / n % 1) * n);
...
function unTrueSkytale(n, input) {
    const uncryptArray = [];
    let cycle = Math.ceil(input.length / n);
    for (let i = 0; i < n; i++) {
        const hereArray = [];
...
*/
//# sourceMappingURL=Skytale.js.map