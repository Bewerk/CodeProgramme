...
function onToFile() {
    let obParameter = document.getElementById('obDateiMitParameter').value;
    let inhalt = '';
    if (obParameter == 'ohne') {
        ...
        let alphabet = parameterObjekt.alphabet;
        let keyword = parameterObjekt.keyword;
        let keynumbers = parameterObjekt.keynumbers;
        let antwort = parameterObjekt.input;
        inhalt = alphabet + '\r\n' + keyword + '\r\n' + keynumbers + '\r\n' + antwort;
    }
...
async function onFileToEncrypt() {
    const fileInput = document.getElementById('Dateieingabe');
    ...
    let alphabet = liste[0];
    let keyword = liste[1];
    let keynumbers = liste[2];
    /*
    let keyObject = {
        alphabet: alphabet,
        keyword: keyword,
    ...
    */
    ...
    document.getElementById('InputFeld').value = input;
    let richtung = document.getElementById('DateiVerarbeitungsRichtung').value;
    if (richtung == 'encrypt') {
        onSpreizung();
    }
...
async function onXmlFileInput() {
    ...
    const fileInput = document.getElementById('xmlDateieingabe');
...
function onXmlFileInputHttpRequestTest() {
    ...
    const fileInput = document.getElementById('xmlDateieingabe');
    const xmlFile = document.getElementById('xmlDateieingabe').files[0];
    ...
    function processTheXmlDoc(xmlObject) {
...
function onToXmlFile() {
    ...
    */
    ...
    let keyword = parameterObjekt.keyword;
    let keynumbers = parameterObjekt.keynumbers;
    let input = parameterObjekt.input;
    let whichway = parameterObjekt.whichway;
    ...
    */
    ...
    xmlHttpReqObject.send();
    */
    let xmlAsString = new XMLSerializer().serializeToString(xmlDoc.documentElement);
...
//# sourceMappingURL=CheckerDateienTest.js.map