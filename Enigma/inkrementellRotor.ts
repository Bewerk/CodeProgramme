let zSatz: string ='ABCDEFGHIJKLMNOPQRSTUVWXYZ';
let ro1: string = 'BDFHJLCPRTXVZNYEIWGAKMUSQO';
let ro2: string = 'AJDKSIRUXBLHWTMCQGZNPYFVOE';
let ro3: string = 'EKMFLGDQVZNTOWYHXUSPAIBRCJ';
...
function setCharSet(input:string):string {
...
function fisherYates(inputArray: string[]) {
...
function checkInput(input: string, charSet:string): string {
...
function checkRotor(charSet: string, rotor: string): boolean {
...
function enigma (key: number, input: string, encrypt: boolean): string {
    ...
    let zeichen: string = zSatz;
    let rotor1: string = ro1;
    let rotor2: string = ro2;
    let rotor3: string = ro3;
    let missingcharstring: string = checkInput(input, zeichen);
    ...
    if (encrypt) {
        let caesarChiffriert:string = incrementalCaesar(key, input, zeichen);
        let gewälzt1: string = rotorEncrypt(caesarChiffriert, rotor1, zeichen);
        let gewälzt2: string = rotorEncrypt(gewälzt1, rotor2, zeichen);
        let gewälzt3: string = rotorEncrypt(gewälzt2, rotor3, zeichen);
        console.log(input, caesarChiffriert, gewälzt1, gewälzt2, gewälzt3);
        
        return gewälzt3;
    }
    else {
        let rückWalze3: string = rotorEncrypt(input, zeichen, rotor3);
        let rückWalze2: string = rotorEncrypt(rückWalze3, zeichen, rotor2);
        let rückWalze1: string = rotorEncrypt(rückWalze2, zeichen, rotor1);
        let dechiffriert: string = decryptIncrementalCaesar(key, rückWalze1, zeichen);
...
function incrementalCaesar (n: number, input: string, alphabet:string): string {
    const encryptArray: string[] = [];
    for (let i=0; i<input.length; i++) {
        let tocountfrom: number = alphabet.indexOf(input[i]);
        let topush: number = tocountfrom + n;
...
function decryptIncrementalCaesar (n: number, input: string, alphabet: string): string {
    const encryptArray: string[] = [];
    for (let i=0; i<input.length; i++) {
        let topush: number = alphabet.indexOf(input[i]) - n;
        while (topush < 0) {
...
function rotorEncrypt(input:string, rotor: string, alphabet: string): string {
    const encryptArray: string[] = [];
    for (let i=0; i<input.length; i++) {
...
function whatisro1(): string {
    return ro1;
}
function whatisro2():string {
    return ro2;
}
function whatisro3():string {
    return ro3;
}


// Noch nicht fertig... jetzt wahrscheinlich schon.
function changeRotor(newRotor: string, index: number): boolean {
...
function resetCharSetRotors():boolean {
