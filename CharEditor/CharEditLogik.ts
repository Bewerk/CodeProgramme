const Charakter: CharakterObjekt = new CharakterObjekt;

function getAttribute(): CharakterObjekt {
    return Charakter;
}

function calcStep(wert: number, plus: boolean): number {
    if (plus) {
        wert -= 9;
    }
    else {
...
function changeAttributsWert(givenAttribut: Attribut, plus: boolean) {
    const Attribute = getAttribute();
    let wert: number = Attribute.getAttribut(givenAttribut);
    let step: number = calcStep(wert, plus);
...
function setVolksBoni(volk: string) {
    const Attribute = getAttribute();
    Attribute.Volk = volk;
    const VolksBoniArray: number[][] = getVolksBoniArray();
    let volkIndex: number = Volk[volk as keyof typeof Volk];
...
function setCharKlasse(klasse: string) {
    const Attribute = getAttribute();
